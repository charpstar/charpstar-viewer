...
export default function ClientPage() {
  ...
  const [globalCacheTimestamp, setGlobalCacheTimestamp] = useState<
    number | null
  >(null);
  ...
  const handleConfirmModelChange = () => {
    if (pendingModelChange) {
      console.log(
        "Model changed to:",
        pendingModelChange.name,
        pendingModelChange.url
      );
  ...
  const handleConfirmedSave = async () => {
    ...
    try {
      ...
      if (resourceData.gltf && typeof resourceData.gltf === "string") {
        ...
        console.log(
          "Using complete GLTF from saveGLTF:",
          gltfData.length,
          "characters"
        );
      } else {
      ...
      if (gltfData) {
        ...
        try {
          ...
          const listResponse = await fetch(
            `/api/list-uploads?client=${clientName}`
          );
          if (listResponse.ok) {
      ...
      if (successCount === expectedUploads && expectedUploads > 0) {
        ...
        console.log(
          `${successCount}/${expectedUploads} files saved successfully`
        );
        setSaveProgress(100);
        setSaveMessage(
          `Partially completed: ${successCount}/${expectedUploads} resources saved.`
        );
