...
import { KHRMaterialsVariants, KHRTextureBasisu, KHRTextureTransform, KHRMaterialsSheen, KHRMaterialsTransmission, KHRDracoMeshCompression } from '@gltf-transform/extensions';
...
async function convertToGlb(buffer: Buffer, sourceUrl: string, isGlb: boolean, variantName?: string | null): Promise<Buffer> {
  // Use ALL_EXTENSIONS to handle all possible extensions including Draco
  const readIO = new NodeIO().registerExtensions(ALL_EXTENSIONS);
  ...
  // Remove Draco extension from the document (geometry already decompressed by readIO)
  const root = doc.getRoot();
  const usedExts = root.listExtensionsUsed();
  const reqExts = root.listExtensionsRequired();
  
  for (let i = usedExts.length - 1; i >= 0; i--) {
    if (usedExts[i].extensionName === 'KHR_draco_mesh_compression') {
      usedExts.splice(i, 1);
    }
  }
  for (let i = reqExts.length - 1; i >= 0; i--) {
    if (reqExts[i].extensionName === 'KHR_draco_mesh_compression') {
      reqExts.splice(i, 1);
    }
  }
